...
def stuInput():
    while True:
        print('[ 학생성적입력 ]')
        stuname= input('학생이름을 입력하세요.(0 입력시 종료)')
        if stuname=='0':
            break
        kor= int(input('국어점수를 입력해봐요..>'))
        eng= int(input('영어점수를 입력해봐요..>'))
...
def stuSearch():
    ...
    for stu in stuSave:
        if stu ==sname:
            ...
            break
    if count ==0:
        print('입력하신 이름의 학생이 없습니다. 다시입력해주세요.')        
...
def stuModify():
    ...
    for stu in stuSave:
        if stu == sname:
            ...
            print('1. 국어점수')
            print('2. 영어점수')
            tempNum= int(input('수정할 과목을 선택하세요...>'))
            if tempNum==1:
